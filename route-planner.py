...
class GraphNode:
    ...
    def add_child(self, node, distance):
        for edge in self.edges:
            if edge.node == node:
                #print("Node edge already exists")
                return -1
...
class Graph():
    def __init__(self, node_list):
        ...
        self.node_dict = {node.value: node for node in self.nodes}
...
def calculate_h_value(current_node_coordinates, goal_node_coordinates):
    #Euclidian Distance base algorithm credit to GeeksforGeeks
    x1, y1 = current_node_coordinates
